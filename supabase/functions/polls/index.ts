  ...
  try {
    ...
    if (segments.length === 0 && req.method === "GET") {
      const { data, error } = await supabase
        .schema("content")
        .from("polls")
        .select("id, question, closes_at, created_at, options:poll_options(id, label, display_order)")
    ...
    if (segments.length === 0 && req.method === "POST") {
      ...
      const { data, error } = await supabase.schema("content").rpc("create_poll_with_options", {
