...
const SELECT_COLUMNS = "id, title, slug, body, author_id, status, published_at, created_at, updated_at";
...
serve(async (req) => {
  ...
  try {
    ...
    if (segments.length === 1 && req.method === "PATCH") {
      ...
      if (!data) return errorResponse("not_found", "No news item with that id (or not authorized).", 404);
      return jsonResponse(data);
    }
    ...
    if (segments.length === 1 && req.method === "DELETE") {
      ...
      if (error) return errorResponse("query_error", error.message, 500);
      if (!count) return errorResponse("not_found", "No news item with that id (or not authorized).", 404);
