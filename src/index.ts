export { configureCoreverseClient, CoreverseApiError } from "./client/http.ts";
export type { CoreverseClientConfig, CoreverseErrorBody } from "./client/http.ts";

// The lines below are what `npm run generate` (Orval, see orval.config.ts)
...
// run `npm install && npm run generate` locally and can see what landed
